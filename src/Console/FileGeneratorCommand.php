        ...
        InputInterface $input, OutputInterface $output): void
    {
        ...
        if ($lines <= 0) {
            ...
            $question = new Question(
                'Сколько строк необходимо сгенерировать?');
        ...
        $output->writeln(sprintf(
            'Подождите, идет генерация файла <info>%s</info>',
            $file));
        ...
        if ($size !== null) {
            $output->writeln(sprintf(
                'Спасибо за ожидание. Итоговый размер файла: <info>%d байт</info>',
                $size
            ));
        } else {
            $output->writeln(
                '<error>Во время генерации видимо произошла ошибка</error>');
        }
    }
}
